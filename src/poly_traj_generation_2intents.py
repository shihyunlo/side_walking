...
def PolyTrajGeneration(time_shift,rob_pos, rob_vel, rob_intent, ped_pos, ped_vel,local_traj_duration, dt, time, reaction_time, max_accelx, max_accely, timing_sm, safety_sm, ped_goal, rob_goal, vh, vr, n_states):
    ...
    max_accelx = abs(max_accelx)
    delta_accel = 0.05

    #intent1 implementation
    if rob_intent==1:
        max_accelx = -max_accelx
        delta_accel = -delta_accel
        x_sampled_accel_lower_bound = -x_sampled_accel_lower_bound
        x_sampled_accel_lower_bound = min(x_sampled_accel_lower_bound,0)
    elif rob_intent==0:
